...
> **版本**: v0.1 | **日期**: 2026-02-27 | **定位**: 面向 AI Coding 产品的细粒度代码评测指标体系构建方案，覆盖 SE 全生命周期，包含指标定义、实现路径、资源需求与可行性评估
...
**定义 & 公式**：通过"逆向生成"技术评估代码对需求的忠实度。给定需求 R，模型生成代码 C，再由 LLM 从 C 反向重建需求 R'，比较 R 与 R' 的一致性。

```
SBC = 0.7 × cosine_sim(embed(R), embed(R')) + 0.1 × BLEU(R, R') + 0.2 × completeness(R, R')
completeness = max(0, 1 - (|missing| + |extra|) / max(|keywords_union|, 1))
```

其中 missing 为 R 有但 R' 缺失的关键词（功能遗漏），extra 为 R' 多出的关键词（幻觉信号）。

| 要素 | 说明 |
|------|------|
| 实现路径 | 半自动：LLM 逆向生成 + embedding 计算 |
| 工具链 | sentence-transformers (all-MiniLM-L6-v2)、sacrebleu、spaCy（关键词提取） |
| 数据需求 | 需求-代码对。可从 BigCodeBench（复杂指令）、SWE-bench（issue 描述）复用，也可自建 |
| 人力需求 | 1 人开发 pipeline（约 1 周），无需标注 |
| 落地难度 | ★★☆☆☆ |
| 优先级 | P1 |
...
**定义 & 公式**：生成代码满足显式约束的比例。约束类型包括：语言指定、复杂度限制、禁用 API、输出格式、库版本要求等。
...
| 数据需求 | 设计稿-代码对。ComUIBench (2602.19276) 提供前端 UI 数据集 |
| 人力需求 | 1-2 人开发 pipeline，视觉评估部分可自动化 |
| 落地难度 | ★★★☆☆ |
| 优先级 | P2 |
| 参考 | ComUIBench, 2602.19276; Design2Code, Si et al. |
...
```
NET = T_generated / T_canonical
```

NET < 1 表示生成代码比参考实现更快，NET > 1 表示更慢。

**NTMU (Normalized Total Memory Usage)**：

```
NTMU = M_generated / M_canonical
```
...
| 数据需求 | TestGenEval (Meta, 2602.05709) 提供 68,647 个函数级测试生成任务；也可基于现有代码库自建 |
| 人力需求 | 1 人搭建测试执行 pipeline（约 1 周） |
| 落地难度 | ★☆☆☆☆ |
| 优先级 | P0 |
| 参考 | Alagarsamy et al., 2023 (TestGen-LLM, Meta); Varshney et al., 2602.05709 (TestGenEval) |
...
| 落地难度 | ★★★☆☆ |
| 优先级 | P2 |
| 参考 | Deursen et al., 2001 (测试异味); Varshney et al., 2602.05709 |
...
```
defect_density = (静态分析缺陷 + 动态测试缺陷) / (代码行数 / 10000)
```
...
| 数据需求 | AACR-Bench (2602.12679) 提供来自 GitHub PR 的审查数据 |
| 人力需求 | 3-5 名开发者标注审查意见质量（约 2-4 周），需定义标注规范 |
| 落地难度 | ★★★★☆ |
| 优先级 | P2 |
| 参考 | AACR-Bench, 2602.12679; SWR-Bench, 2602.11590 |
...
| 5.1 | 输出一致性 | 通用 | 全自动 | 多次采样 + scipy | 任意 benchmark | 0.5 | ★ | P1 |
| 5.2 | 数据污染鲁棒性 | 通用 | 半自动 | LiveCodeBench | LiveCodeBench | 5 | ★★ | P1 |
| 5.3 | 生成成本 | 通用 | 全自动 | API 日志 | 无 | 0.5 | ★ | P0 |
| 5.4 | 价值观对齐 | 通用 | 半自动 | CyberSecEval 2 | CyberSecEval 2 | 5 | ★★ | P1 |
...
```
Phase 1（第 1-2 月）：全自动指标基础设施
├── 搭建沙箱执行环境（Docker + cgroups）
├── 集成 pass@k / 部分正确性 / 效率指标
├── 配置 Lint / 复杂度 / 重复率 / 缺陷密度
├── 集成 SAST 安全扫描（Bandit + Semgrep）
├── 搭建覆盖率 / 回归测试 pipeline
├── 集成生成成本统计
└── 产出：~15 个全自动指标上线

Phase 2（第 2-4 月）：半自动指标 + LLM-as-Judge 校准
├── 开发 SBC 逆向生成 pipeline
├── 开发可读性评估（结构特征 + LLM-as-Judge）
├── 配置风格一致性 / 代码自然性计算
├── 搭建许可证合规扫描
├── 建立 LLM-as-Judge 评分一致性校准流程
├── 编写 AI 特有异味 Semgrep 规则
└── 产出：~10 个半自动指标上线，覆盖工程质量全貌

Phase 3（第 4-6 月）：人工标注驱动指标 + 体系完善
├── 构建歧义需求标注数据集
├── 构建代码审查质量标注数据集
├── 开发重构质量评测 pipeline
├── 集成跨语言/跨领域泛化评测
├── 全指标体系联调与报告生成
└── 产出：完整指标体系上线，支持一键评测报告
```
...
| 阶段 | 人力 | 工作内容 |
|------|------|---------|
| Phase 1 | 2 人 × 2 月 | 基础设施 + 全自动指标 |
| Phase 2 | 2-3 人 × 2 月 | 半自动指标 + 校准 |
| Phase 3 | 3-5 人 × 2 月 | 标注 + 人工驱动指标 + 联调 |
| 持续运营 | 1 人 | 数据更新、指标维护、新指标接入 |
...
### 安全性与许可证合规
21. Siddiq & Santos, 2022. "SecurityEval Dataset". arXiv:2212.09520
...
31. Varshney et al., 2026. "TestGenEval". arXiv:2602.05709
...
### 代码审查
41. "AACR-Bench", Feb 2026. arXiv:2602.12679
42. "SWR-Bench", Feb 2026. arXiv:2602.11590
